...
err_t ICACHE_FLASH_ATTR tcpsrv_received_data_default(TCP_SERV_CONN *ts_conn,
        uint8 *data_ptr, uint16 data_cntr)
{
#if DEBUG_LEVEL > 1
	tcpsrv_print_remote_info(ts_conn);
...
{
	if ((ts_conn->flag.throttle_data_reception)
 ...
 * Returns      : none
 *******************************************************************************/
err_t ICACHE_FLASH_ATTR tcpsrv_sent_data(TCP_SERV_CONN * ts_conn, uint8 *psent,
        uint16 length)
{
...
static void ICACHE_FLASH_ATTR tcpsrv_close_cb(TCP_SERV_CONN * ts_conn)
{
	...
	{
		if (++ts_conn->recv_check > TCP_SRV_CLOSE_WAIT)
		{ // ts_conn->pcfg->time_wait_cls * 2 + 4) {
#if DEBUG_LEVEL > 2
			tcpsrv_print_remote_info(ts_conn);
...
bool tcpsrv_close(TCP_SERV_CFG *p)
{
	...
	while (p->conn_links != NULL)
	{
		...
		ts_conn->state = ESPCONN_CLOSE;
		if (phcfg->func_discon_cb != NULL)
			phcfg->func_discon_cb(ts_conn);
		tcpsrv_list_delete(ts_conn);
